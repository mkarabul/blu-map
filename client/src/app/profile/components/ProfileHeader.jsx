...
export default function ProfileHeader({
  ...
  useEffect(() => {
    ...
    const checkFollowStatus = async () => {
      if (user && userName) {
        try {
         const response2 = await fetch(`http://localhost:5000/api/admin/${user?.sub}`);
        if (!response2.ok) {
        ...
        if (response.ok) {
          ...
          const isFollowing = followingUsers.some(follower => follower.userName === dataUserName);
  ...
  const followUnfollowButton = isFollowing ? (
    <button onClick={handleUnfollow} className="btn-unfollow transition duration-150 ease-in-out">
